...
- Live API Env Vars: for your current setup, use `API_FOOTBALL_KEY` from API-SPORTS for fixtures and `THE_ODDS_API_KEY` from The Odds API for betting odds. Basketball is optional.
  ...
  - `THE_ODDS_API_KEY`: odds key from The Odds API; used for football 1X2 odds.
...
- Env Var: `NEXT_PUBLIC_API_URL` should point to the active backend.
...
Rotate any database credential pasted into chat before production. This repo only includes `.env.example`; real secrets stay in server environment variables.

## Notes on original bot requirements

The project was pivoted from Telegram bot delivery to a Vercel/AWS website. Telegram dependencies are still listed for future extension, but the active delivery channel is the website. Fake proof, fixed signals, and deceptive winner-only history are intentionally not implemented.
