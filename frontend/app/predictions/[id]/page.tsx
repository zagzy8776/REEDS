    ...
    return (
      <main className="mx-auto max-w-4xl px-6 py-10">
        ...
        <div className="card mt-6">
          ...
          <p className="mt-2 text-slate-400">It may have been removed or replaced with a newer pick.</p>
        </div>
      </main>
    );
  }
    ...
    <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6 sm:py-10">
      ...
      <section className={`card mt-6 overflow-hidden ${premiumLocked ? "premium-card" : ""}`}>
        ...
        <div className="mt-8 grid gap-5 md:grid-cols-3">
          <div className="rounded-2xl border border-emerald-400/20 bg-emerald-400/10 p-5 md:col-span-2">
            <p className="text-sm text-emerald-100/80">The pick</p>
      ...
      <section className="mt-6 card">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="badge inline-block">Why this pick</p>
            <h2 className="mt-3 text-2xl font-black">What the model is seeing</h2>
            <p className="mt-2 max-w-3xl text-sm text-slate-400">{analysis.summary || "The model checks team form, goals, market price, and risk before showing a pick."}</p>
          </div>
          <p className="rounded-xl border border-emerald-400/20 bg-emerald-400/10 px-3 py-2 text-sm text-emerald-200">{analysis.market_logic || "More market notes will show after the next refresh."}</p>
        </div>
        <div className="mt-5 grid gap-3 md:grid-cols-3">
          {(factors.length ? factors : [
            { label: "Form", value: "-", note: "Recent team form" },
            { label: "Goals", value: "-", note: "Scoring trend" },
            { label: "Risk", value: prediction.risk_level, note: "How risky the pick looks" },
          ]).map((factor: any) => (
      ...
      <section className="mt-6 grid gap-6 lg:grid-cols-2">
        <div className="card">
          <h2 className="text-xl font-bold">Pick notes</h2>
          <p className="mt-3 text-slate-300">{prediction.engine_summary || "This pick uses match context, past results, team form, and market price."}</p>
        ...
        <div className="card">
          <h2 className="text-xl font-bold">Odds history</h2>
          <p className="mt-2 text-sm text-slate-400">Opening and closing prices help show whether the pick beat the market.</p>
          <div className="mt-4 overflow-x-auto">
            <table className="w-full text-left text-sm">
              ...
              <tbody>
                ...
                )) : <tr><td className="py-3 text-slate-400" colSpan={5}>No odds history for this pick yet.</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
      ...
      <section className="mt-6 grid gap-6 lg:grid-cols-2">
        <div className="card">
          <h2 className="text-xl font-bold">What the room thinks</h2>
          <p className="mt-2 text-sm text-slate-400">See where other users are leaning on this match.</p>
          <div className="mt-4 space-y-3">
            ...
            )) : <p className="text-sm text-slate-400">No community picks here yet.</p>}
          </div>
        </div>
          ...
          <div className="mt-4 max-h-80 space-y-3 overflow-y-auto">
            {(prediction.community?.entries || []).length ? prediction.community.entries.map((entry: any) => (
              <div key={entry.id} className="rounded-xl border border-slate-800 bg-slate-950 p-4">
                ...
                <p className="mt-2 text-sm text-slate-300">{entry.analysis_text || "No note added."}</p>
                <CommunityActions seed={entry.id % 7} />
              </div>
            )) : <p className="text-sm text-slate-400">Be the first to post a take on this match.</p>}
          </div>
