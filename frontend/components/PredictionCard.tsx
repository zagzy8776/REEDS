    ...
    <div className={`card relative overflow-hidden ${premiumLocked ? "premium-card" : ""}`}>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
        <div className="min-w-0">
          ...
          <h3 className="mt-1 break-words text-base font-bold sm:text-lg">{p.home_team} vs {p.away_team}</h3>
        </div>
        <span className={`${premiumLocked ? "premium-lock" : "badge"} w-fit shrink-0`}>{premiumLocked ? "🔒 " : ""}{p.confidence}% EDGE</span>
      </div>
      <div className="mt-4 rounded-xl bg-slate-950 p-4">
        <p className="text-sm text-slate-400">{p.market}</p>
        <p className="break-words text-xl font-black text-emerald-300 sm:text-2xl">{p.pick}</p>
      </div>
