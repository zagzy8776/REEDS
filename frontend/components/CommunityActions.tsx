  ...
  return (
    <div className="mt-3 space-y-3">
      <div className="flex flex-wrap gap-2 text-xs font-bold">
        ...
        <button type="button" onClick={() => setText((x) => x || "I like this because ")} className="rounded-full border border-slate-700 bg-slate-950/70 px-3 py-2 text-slate-200">
          💬 Reply
        </button>
      ...
      {text || replies.length ? (
        <div className="rounded-2xl border border-slate-800 bg-slate-950/70 p-3">
          ...
          <div className="flex gap-2">
            <input value={text} onChange={(e) => setText(e.target.value)} placeholder="Drop a quick reply..." className="min-w-0 flex-1 rounded-xl border border-slate-800 bg-slate-950 px-3 py-2 text-sm" />
          ...
          <p className="mt-2 text-[11px] text-slate-500">Quick replies stay on this page for now.</p>
        </div>
      ) : null}
    </div>
  );
