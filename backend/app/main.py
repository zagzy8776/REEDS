...
def api_stats_summary():
    ...
    try:
        ...
        rows = db.query(Prediction, Fixture).join(Fixture, Prediction.fixture_id == Fixture.id).filter(Prediction.is_published == True, Fixture.home_score != None, Fixture.away_score != None).order_by(Fixture.match_date.desc()).limit(12000).all()
        by_sport = defaultdict(lambda: {"wins": 0, "total": 0})
        by_market = defaultdict(lambda: {"wins": 0, "total": 0})
        for pred, fx in rows:
            by_sport[fx.sport or "unknown"]["total"] += 1
            by_market[pred.market or "unknown"]["total"] += 1
            won = pred.engine_meta.get("outcome", {}).get("result") == "won" if isinstance(pred.engine_meta, dict) else False
            if won:
                by_sport[fx.sport or "unknown"]["wins"] += 1
                by_market[pred.market or "unknown"]["wins"] += 1
        by_sport_out = [{"sport": k, "wins": v["wins"], "total": v["total"], "hit_rate": round((v["wins"] / v["total"]) * 100, 1) if v["total"] else 0.0} for k, v in sorted(by_sport.items())]
        by_market_out = [{"market": k, "wins": v["wins"], "total": v["total"], "hit_rate": round((v["wins"] / v["total"]) * 100, 1) if v["total"] else 0.0} for k, v in sorted(by_market.items())]
        tracked_bets = 0
        profit_units = 0.0
        clv_tracked = 0
        positive_clv = 0
        ...
        market_proof = {"tracked_bets": tracked_bets, "profit_units": round(profit_units, 2), "roi_percent": round((profit_units / tracked_bets) * 100, 2) if tracked_bets else 0, "clv_tracked": clv_tracked, "positive_clv_rate": round((positive_clv / clv_tracked) * 100, 2) if clv_tracked else 0, "by_market": [{"market": r["market"], "bets": r["bets"], "profit": round(r["profit"], 2), "roi_percent": round((r["profit"] / r["bets"]) * 100, 2) if r["bets"] else 0, "clv_total": r["clv_total"], "positive_clv_rate": round((r["clv_positive"] / r["clv_total"]) * 100, 2) if r["clv_total"] else 0} for r in sorted(market_proof_rows.values(), key=lambda x: x["market"])], "note": "ROI is calculated as flat 1-unit staking on recent supported settled markets. CLV requires matching closing odds snapshots."}
        latest_times = db.query(ModelVersion.sport.label("sport"), func.max(ModelVersion.trained_at).label("trained_at")).group_by(ModelVersion.sport).subquery()
        model_rows = db.query(ModelVersion).join(latest_times, and_(ModelVersion.sport == latest_times.c.sport, ModelVersion.trained_at == latest_times.c.trained_at)).order_by(ModelVersion.sport.asc()).limit(24).all()
        backtests = db.query(BacktestRun).order_by(BacktestRun.created_at.desc()).limit(20).all()
        odds_snapshots_count = db.query(OddsSnapshot).count()
        return {"results": {"settled_picks": settled_predictions, "wins": recent_wins, "losses": recent_losses, "hit_rate": round(recent_accuracy, 1) if recent_accuracy is not None else 0, "by_sport": by_sport_out, "by_market": by_market_out, "confidence_buckets": confidence_buckets}, "market_proof": market_proof, "backtests": [{"id": r.id, "sport": r.sport, "model_type": r.model_type, "sample_size": r.sample_size, "accuracy": r.accuracy, "brier_score": r.brier_score, "log_loss": r.log_loss, "created_at": r.created_at} for r in backtests], "models": [{"id": m.id, "sport": m.sport, "type": m.model_type, "sample_size": m.sample_size, "accuracy": m.accuracy, "active": m.is_active, "trained_at": m.trained_at} for m in model_rows], "data_quality": {"odds_snapshots": odds_snapshots_count}}
    except Exception as exc:
