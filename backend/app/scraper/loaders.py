    ...
    if football_key:
        try:
            ...
            for item in payload.get("response", []) or []:
                ...
                existing = db.query(Fixture).filter(Fixture.sport == "soccer", Fixture.match_date == match_date_raw.date(), Fixture.home_team == home_name, Fixture.away_team == away_name).first()
                if existing:
    ...
    if basketball_key:
        try:
            ...
            for item in payload.get("response", []) or []:
                ...
                existing = db.query(Fixture).filter(Fixture.sport == "basketball", Fixture.match_date == match_date_raw.date(), Fixture.home_team == home_name, Fixture.away_team == away_name).first()
                if existing:
                    existing.home_score = home_score
                    existing.away_score = away_score
                    extra = dict(existing.extra or {})
    ...
    if len(effective_keys) <= 6:
        try:
            ...
            if isinstance(all_sports, list):
                discovered = [s["key"] for s in all_sports if isinstance(s, dict) and s.get("active") and s.get("key")]
                if discovered:
    ...
    for sport_key in effective_keys:
        ...
        for event in events:
            try:
                ...
                existing = db.query(Fixture).filter(Fixture.sport == canonical_sport, Fixture.match_date == match_date, Fixture.home_team == home_resolved, Fixture.away_team == away_resolved).first()
                if existing:
